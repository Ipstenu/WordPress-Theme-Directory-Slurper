 ...
 * $m_theme1, and $message2 may contain a "%s" placeholder to be filled in
 * with $m_theme2.
 *
 * If necessary, $m_theme1 and $m_theme2 (if present) will be truncated to
...
function fit_message(
	...
	$length1 = strlen( sprintf( $message1, $m_theme1 ) );
	if ( ! $message2 && $length1 <= $length ) {
		// Easy case: one message, short enough
		return sprintf( $message1, $m_theme1 );

	} else if ( ! $message2 ) {
		// One message, too long
		$m_theme1 = truncate_text(
			$m_theme1,
			strlen( $m_theme1 ) - ( $length1 - $length )
		);
		return sprintf( $message1, $m_theme1 );
		...
		if ( $needed > 0 ) {
			$m_theme1 = truncate_text(
				$m_theme1,
				strlen( $m_theme1 ) - $needed
			);
		}
		$message1 = sprintf( $message1, $m_theme1 );
		$length += 2; // reset max length

	} else {
		// Two messages, see if we need to truncate them
		$length -= 2; // account for padding between messages
		$length2 = strlen( sprintf( $message2, $m_theme2 ) );
		...
		$m_theme1 = truncate_text(
			$m_theme1,
			strlen( $m_theme1 ) - ( $length1_orig - $length1 )
		);
		$m_theme2 = truncate_text(
			$m_theme2,
			strlen( $m_theme2 ) - ( $length2_orig - $length2 )
		);
		$message1 = sprintf( $message1, $m_theme1 );
		$message2 = sprintf( $message2, $m_theme2 );
		$length += 2; // reset max length
	}
